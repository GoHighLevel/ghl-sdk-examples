...
def webhook():
    """Handle GHL webhook - run async operation in event loop"""
    try:
        async def async_webhook_operations():
            await webhook_middleware(request)
            # print('Received webhook data:', request.body())
        ...
        result = run_async_in_loop(async_webhook_operations())
        ...
        traceback.print_exc()
        return {'error': f'Error processing webhook: {str(error)}'}, 500
